...
export default function (meta) {
  return class Plugin {
    ...
    load() {
      const currentVersionInfo = Utilities.loadData(
        this.getName(),
        "currentVersionInfo",
        { version: this.getVersion(), hasShownChangelog: false }
      );
      if (
        currentVersionInfo.version != this.getVersion() ||
    ...
    showSettingsModal() {
      ...
      Modals.showModal(
        ...
        ReactTools.createWrappedElement(this.getSettingsPanel()),
        {
          cancelText: "",
          confirmText: "Done",
    ...
    buildSetting(data) {
      ...
      let setting = null;
      if (type == "color")
        setting = new Settings.ColorPicker(name, note, value, onChange, {
          disabled: data.disabled,
          presetColors: data.presetColors,
        });
      else if (type == "dropdown")
        setting = new Settings.Dropdown(
          name,
          note,
          value,
          data.options,
        ...
        setting = new Settings.RadioGroup(
          name,
          note,
          value,
          data.options,
        ...
        setting = new Settings.Switch(name, note, value, onChange, {
          disabled: data.disabled,
        });
      else if (type == "textbox")
        setting = new Settings.Textbox(name, note, value, onChange, {
    ...
    buildSettingsPanel() {
      ...
      const buildGroup = (group) => {
        ...
        const settingGroup = new Settings.SettingGroup(name, {
          shown,
          collapsible,
        }).append(...list);
        settingGroup.id = id;
