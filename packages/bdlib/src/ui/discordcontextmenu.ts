...
export default class DiscordContextMenu {
  ...
  static buildMenuItem(props) {
    ...
    if (type === "submenu") {
      if (!props.children)
        props.children = this.buildMenuChildren(props.render || props.items);
  ...
  static buildMenu(setup) {
    return (props) => {
      return ce(ContextMenu.default, props, this.buildMenuChildren(setup));
    };
  }
  ...
  static openContextMenu(event, menuComponent, config) {
    return ContextMenuActions.openContextMenu(
      ...
      function (e) {
        return ce(
          menuComponent,
          Object.assign({}, e, { onClose: ContextMenuActions.closeContextMenu })
        );
      },
      config
    );
  ...
  static getDiscordMenu(nameOrFilter) {
    ...
    const directMatch = WebpackModules.getModule(
      (m) => m.default && nameOrFilter(m.default)
    );
  ...
  static forceUpdateMenus() {
    const menus = document.querySelectorAll(
      `.${DiscordClasses.ContextMenu.menu.first}`
    );
    for (const menu of menus) {
      const stateNode = Utilities.findInTree(
        ReactTools.getReactInstance(menu),
  ...
  static patchMenuItem() {
    const MenuItem = WebpackModules.getModule(
      (m) => m.default && m.default.displayName == "MenuItem"
    );
  ...
  static patchToggleItem() {
    const MenuToggleItem = WebpackModules.getModule(
      (m) => m.default && m.default.displayName == "MenuCheckboxItem"
    );
