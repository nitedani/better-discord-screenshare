...
export default class Tooltip {
  ...
  constructor(node, text, options = {}) {
    ...
    if (!classExists(this.side))
      return Logger.err("Tooltip", `Side ${this.side} does not exist.`);
    if (!classExists(this.style))
      return Logger.err("Tooltip", `Style ${this.style} does not exist.`);

    this.element = DOMTools.createElement(
      `<div class="${DiscordClasses.TooltipLayers.layer}">`
    );
    this.tooltipElement = DOMTools.createElement(
      `<div class="${DiscordClasses.Tooltips.tooltip} ${getClass(
        this.style
      )}"><div class="${
        DiscordClasses.Tooltips.tooltipPointer
      }"></div><div class="${DiscordClasses.Tooltips.tooltipContent}">${
        this.label
      }</div></div>`
    );
    ...
    if (this.disablePointerEvents) {
      this.element.classList.add(
        DiscordClasses.TooltipLayers.disabledPointerEvents
      );
      this.tooltipElement.classList.add(
        DiscordClasses.Tooltips.tooltipDisablePointerEvents
      );
    }
    if (this.isTimestamp)
      this.tooltipElement.classList.add(
        WebpackModules.getByProps("timestampTooltip").timestampTooltip
      );
  ...
  get container() {
    return document.querySelector(
      DiscordSelectors.App.app.sibling(
        DiscordSelectors.TooltipLayers.layerContainer
      )
    );
  }
  ...
  show() {
    ...
    this.tooltipElement.className = `${
      DiscordClasses.Tooltips.tooltip
    } ${getClass(this.style)}`;
    if (this.disablePointerEvents)
      this.tooltipElement.classList.add(
        DiscordClasses.Tooltips.tooltipDisablePointerEvents
  ...
  centerHorizontally() {
    const nodecenter =
      this.node.getBoundingClientRect().left + this.node.offsetWidth / 2;
    this.element.style.setProperty(
      "left",
  ...
  centerVertically() {
    const nodecenter =
      this.node.getBoundingClientRect().top + this.node.offsetHeight / 2;
    this.element.style.setProperty(
      "top",
