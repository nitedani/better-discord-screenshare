...
const Markdown = WebpackModules.getModule(
...
export default class Modals {
  ...
  static showModal(title, children, options = {}) {
    ...
    return DiscordModules.ModalActions.openModal((props) => {
      return React.createElement(
        DiscordModules.ConfirmationModal,
        Object.assign(
          {
            header: title,
            confirmButtonColor: danger
              ? DiscordModules.ButtonData.ButtonColors.RED
              : DiscordModules.ButtonData.ButtonColors.BRAND,
  ...
  static showChangelogModal(title, version, changelog, footer) {
    ...
    if (!TextElement)
      return Logger.warn(
        "Modals",
        "Unable to show changelog modal--TextElement not found."
      );
    const changelogItems = [];
    for (let c = 0; c < changelog.length; c++) {
      const entry = changelog[c];
      const type = DiscordClasses.Changelog[entry.type]
        ? DiscordClasses.Changelog[entry.type]
        : DiscordClasses.Changelog.added;
      ...
      const list = ce(
        "ul",
        null,
        entry.items.map((i) => ce("li", null, ce(Markdown, null, i)))
      );
      changelogItems.push(list);
    }
    const renderHeader = function () {
      return ce(
        DiscordModules.FlexChild.Child,
        { grow: 1, shrink: 1 },
        ce(
          DiscordModules.Titles.default,
