...
const AccessibilityProvider = WebpackModules.getByProps(
...
const AnalyticsTrackContext = WebpackModules.find(
  (m) =>
    m._currentValue &&
    m._currentValue.toString &&
    m._currentValue
      .toString()
      .includes("AnalyticsTrackImpressionContext function unimplemented")
);
const AnalyticsTracker = WebpackModules.find(
  (m) => m.toString && m.toString().includes("setDebugTrackedData")
);
...
export default class Popouts {
  ...
  static initialize() {
    ...
    Patcher.before(
      ...
      (_, [element]) => {
        if (element.parentElement === this.layerContainer)
  ...
  static showUserPopout(target, user, options = {}) {
    const {
      position = "right",
      align = "top",
      guild = DiscordModules.SelectedGuildStore.getGuildId(),
      channel = DiscordModules.SelectedChannelStore.getChannelId(),
    } = options;
...
function DiscordProviders({ children, container }) {
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        AnalyticsTrackContext.Provider,
        {
...
function PopoutWrapper({ render, animation, popoutId, ...props }) {
  ...
  return React.createElement(
    Layers.AppReferencePositionLayer,
    Object.assign(props, {
      ref: popoutRef,
      positionKey: "0",
      autoInvert: true,
